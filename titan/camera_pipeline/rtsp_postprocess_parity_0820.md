...
> **[2026-08-31 변경]** 아래 `PlayWorldCameraShake` 호출은 제거됐다. 그건 "누가 쐈는지"를 안
> 보고 월드의 모든 PlayerController(원격 클라 포함)를 거리로만 훑어서, UGV 발사가 자체방호 PC
> 화면까지 흔드는 버그의 원인이었다. 지금은 `PlayImpactCameraShakeForShooterView()` —
> 로컬 플레이어만, 그 뷰타겟이 발사 주체일 때만. 아래 "셰이크가 카메라 매니저 POV에만 걸린다"는
> 설명 자체는 그대로 유효하다. 상세: `rcws/2026-08-31_selfdefense_camera_shake_bugs.md` §2.
...
> **[2026-08-31 수정됨]** 아래의 "`AActor::CalcCamera`로 모디파이어 적용 전 POV를 다시 계산"
> 부분은 **버그였다**. 카메라 매니저의 POV 캐시는 모든 액터 틱이 끝난 뒤에 갱신되므로
> (`LevelTick.cpp`: "Update cameras last... after all actors have been ticked") 액터 틱에서
> 읽으면 항상 직전 프레임 값인데 `CalcCamera()`는 이번 프레임 값이라, 셰이크가 없어도
> "카메라가 지난 한 프레임 동안 움직인 양"이 셰이크로 둔갑해 캡쳐에 실렸다. 리슨서버
> 클라이언트에서 RCWS를 조향하면 환경카메라/CCTV가 떠는 증상으로 나타남.
> 지금은 `UViewShakeProbeCameraModifier`(모디파이어 체인 맨 앞, 자동 설치)가 기록한
> 같은-프레임 POV를 쓴다. 상세: `rcws/2026-08-31_selfdefense_camera_shake_bugs.md` §1.
