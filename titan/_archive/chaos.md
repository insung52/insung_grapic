> [보관됨 2026-08-31] 최신 버전: `M1A2_UGV_Conversion.md`. 사유: 이 문서가 제안한 "스켈레탈
> 메시부터 새로 만들어 AUGVChaosPawn을 직접 구성"하는 접근은 실제 시도 후 디버깅 루프(본
> 오프셋 버그, 서스펜션 붕괴 등)에 빠져 폐기되고, 검증된 M1A2 탱크 BP를 복제하는 방식으로
> 전환됨 — 자세한 배경은 `M1A2_UGV_Conversion.md` §1 참고.
