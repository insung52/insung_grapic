...
2026-09-01 / 완료 / `ADronePawn`이 구 `AUAVPawn`/`BP_UAV`를 대체 — 로터별 물리 비행 + 자율비행 + 짐벌 + 사운드 + 바람 + 탐지단계 + 시나리오 + 리플리케이션 전부 구현됨.
...
- 지형 회피 미구현. 스플라인을 지형 위로 그리는 것으로 대체하고 있다.
...
구현은 `UTargetDetectionComponent`에 추가한 `DetectableFactions`(진영 필터) + 
`MinScreenSizeFraction`(화면에서 이 비율보다 작으면 무시, 드론 기본 0.04). 후자는 너무 먼
거리에서 미리 잡히는 걸 막는다 — 줌인하면 다시 잡히므로 "줌으로 확인한다"는 절차와도 맞는다.
...
| 2026-09-01 | 단일 프로세스 시나리오 전체 검증 완료, 문서 최신화. **2프로세스 실환경 검증만 남음** |
