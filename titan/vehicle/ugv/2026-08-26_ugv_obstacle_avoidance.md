...
선행 문서: `2026-08-27_new_kadex_0811_navmesh_autonomous_driving.md` (내비메시 구축 전반, 3층 구조 설계)
...
- [x] ~~오르막 경사 보상 없음~~ → **2026-08-31 해결.** 다만 여기 적어둔 "최소 스로틀 보장" 방식이
      아니라, 속도 제어기 자체를 PI로 바꿔서 적분항이 경사 보상을 자동으로 하게 했다. 그 결과
      경사 감지 로직(Pitch 측정·필터) 자체가 필요 없어졌다.
      `2026-08-31_ugv-speed-pi-controller.md` 참고 — 같은 문서에 **병목이 제어기에서 구동계
      (토크/기어)로 옮겨갔다**는 실측도 있다.
