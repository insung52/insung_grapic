...
> **후속 문서: `2026-08-26_ugv_obstacle_avoidance.md`** — 이 문서대로 내비메시를 깐 뒤 UGV가
...
> ### ⚠ 2026-08-31 정정 — 위 "필터에서 0.2로 할인"은 **한 번도 동작한 적이 없다**
>
> 엔진이 필터의 이동비용 오버라이드를 **하한 1.0으로 클램프**한다:
> ```cpp
> // Engine/Source/Runtime/NavigationSystem/Private/NavFilters/NavigationQueryFilter.cpp
> void UNavigationQueryFilter::InitializeFilter(...) const {
>     Filter.SetAreaCost(AreaId, FMath::Max(1.0f, AreaData.TravelCostOverride));   // ← 0.2 → 1.0
> }
> ```
> Recast의 A* 휴리스틱이 admissible하려면 area 비용이 1.0 이상이어야 해서 엔진이 의도적으로
> 막아둔 것이다. **필터로는 비용을 올릴 수만 있고 내릴 수 없다.**
>
> 그래서 `UNavQueryFilter_Enemy`의 `AddTravelCostOverride(NavArea_EnemyPath, 0.2f)`는 조용히
> 1.0(중립)이 되어, 이 문서가 쓰인 2026-08-22 이래 적군 경로 가중치가 **줄곧 무효**였다.
> 7절의 미검증 항목("적군이 NavArea_EnemyPath를 실제로 타는지 검증")이 바로 이것이었고,
> 2026-08-31에 사용자가 "보병이 가중치 무시하고 일직선으로 간다"고 리포트하면서 드러났다.
>
> **왜 아무도 못 알아챘나**: `UNavQueryFilter_Infantry`의 두 오버라이드(`NavArea_Road`=1.0,
> `NavArea_UGVBody`=1000000)는 전부 1.0 이상이라 정상 동작했다. 그래서 "필터 오버라이드는 잘
> 먹는다"는 인상만 남았고, 유일하게 1.0 미만인 값 하나만 죽어 있었다.
>
> **UGV의 도로 선호가 멀쩡한 이유도 같은 원리다** — 그건 필터가 아니라 `NavArea_Road` 자신의
> `DefaultCost = 0.2`에 박혀 있고, UGV는 필터를 아예 안 쓰므로 클램프를 거치지 않는다.
>
> **수정(2026-08-31)**: 방향을 뒤집어 "경로를 싸게" 대신 **"경로 밖을 비싸게"**로 바꿨다.
> `UNavQueryFilter_Enemy`가 `NavArea_Default`와 `NavArea_Road`를 `EnemyNavCost::OffPath`(5.0)로
> 올리고, 적군 경로 area는 중립 1.0 그대로 둔다 → 상대 선호도는 원래 의도(5배)와 동일하면서
> 클램프에 안 걸린다. 상세는 `../../ai_combat/2026-08-31_enemy_squad_reorg.md` 5절.
...
생겼고, 커브 선행 감속(제동 곡선)으로 해결했다 → **`2026-08-22_ugv_corner_braking_dev_guide.md`**.
...
- [x] 적군이 `NavArea_EnemyPath`를 실제로 타는지 검증 → **안 타고 있었음**(2026-08-31).
      원인은 엔진의 필터 비용 하한 클램프 — 3.4절 정정 박스 참고. 수정 후 재검증 필요.
