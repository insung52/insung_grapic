...
상세: `rtsp/`, `camera_pipeline/`. 남은 것: 자체방호축 6스트림 정밀 지연
재측정, 순수 Xorg 세션 검증. **[2026-08-31 원인 확정+코드 수정 완료]** 자체방호축에서 RCWS
...
## 9. 드론(UAV) 물리 재구현 — 완료, 2프로세스 검증만 남음
...
남은 것: **2대 PC 실환경 검증**, 그 후 구 `AUAVPawn`/`BP_UAV`와 폴백 분기 제거, 2·3차 전투지
추격 스플라인 추가(코드는 준비됨, 레벨 작업만). 상세:
`vehicle/drone/drone_flight_dev_guide.md`(레퍼런스),
`vehicle/drone/2026-09-01_drone_replaces_bp_uav.md`(작업 경과).
...
- 미니맵 좌표 오차, 피격음이 항상 UGV 위치에서 나는 버그, Linux 조이스틱 미작동/듀얼모니터
  풀스크린 간헐 실패(`memo.md` 백로그). *UAV 가속/감속 곡선 부자연스러움은 드론 재구현으로
  해소됨(§9).*
