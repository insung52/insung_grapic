> ⚠️ [guide/ 이관 시 경고, 2026-08-31] 2026-06-25 기준 작성. **아카이브가 아니라 guide/로
> 옮김** — `quadcam_usage_guide.md`(재사용 방법 중심)와 겹치지만 이 문서는 컴포넌트 내부
> 아키텍처(`FComponentReference` 패턴, VRAM 누수 버그/수정)까지 다뤄서 여전히 유효한 레퍼런스로
> 판단(2026-08-31, 문서 정리 세션 판단 — tanksim/M1A2 문서와 달리 archive 대상 아님).
> §5 "현재 상태"만 옛날 스냅샷이니 신뢰하지 말 것. 2단계(guide 내용 최신화) 때
> `quadcam_usage_guide.md`와 병합 검토 권장.
