...
- **`DefaultSlot` 몽타주 경로 점검 필요**(2026-08-25 발견): `Hit_Reaction` 몽타주가 타는
  `LayeredBoneBlend_0`의 BranchFilter가 `Spine / blendDepth -1`인데, 엔진 소스상 `BlendDepth < 0`이면
  모든 본 가중치가 0이 됨 — 즉 이 몽타주가 실제로는 화면에 반영되지 않고 있을 가능성이 큼.
  자세한 내용과 AnimGraph 구조는 `enemy_locomotion_animation_pipeline.md` 4절 참고.
