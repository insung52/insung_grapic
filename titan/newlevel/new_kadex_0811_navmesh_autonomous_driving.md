...
- **반경 50cm / 높이 6m** — 인스턴스 스케일 `(1.0, 1.0, 6.0)`, 중심 Z = 나무 밑동 + 300
...
| 대상 | 소나무 | 자작나무 | 소계 |
|---|---|---|---|
| `tree_C_1` (메인 경계) | 738 | 771 | 1,509 |
| `tree2_C_1` (섬) | 47 | 43 | 90 |
| `tree_C_2` (섬) | 64 | 56 | 120 |
| **합계** | | | **1,719** |
...
**해결**: ISM 컴포넌트를 여러 개로 쪼개고 **컴포넌트당 150개 이하**로 나눠 쓴다. `set_properties`는
배열을 통째로 교체하는 방식이라 "이어붙이기"가 안 되므로, 한 배열을 키우는 대신 작은 배열을 여러 개
만들어야 한다. 그래서 `Pine_0`~`Pine_7`, `Birch_0`~`Birch_7`, `Island2_*`, `Island3_*`로 나뉘어 있다.
호출도 X축 슬라이스 단위로 쪼개서 각 호출이 120초 안에 끝나게 했다.

부수 함정 하나 더: **인스턴스를 쓴 뒤에 `staticMesh`를 지정**해야 컴포넌트가 인스턴스 데이터를
제대로 리빌드한다. 순서를 반대로 하면 액터 바운드가 갱신 안 되는 등 어긋난 상태가 된다.
