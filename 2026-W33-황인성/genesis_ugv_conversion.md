...
→ `bUseTopSpeedOverride=true, TopSpeed=11.111 m/s`로 맞추는 게 필요 (Sweep Table 측정
조건과 실주행 설정 일치시키기 위함). **에디터 연결 문제로 아직 미적용 — 다음 세션에서 적용 예정.**
...
| 파라미터 | 최초 | 현재 | 사유 |
|---|---|---|---|
| SkidSteerGain (VehicleSetup) | 1.0 | 0.45 | 자율주행 중 제자리에서 빙글빙글 도는 문제 완화 |
| YawGain | 1.5 | 0.6 | 위와 동일 (좌우 트랙 속도차 과민 반응 완화) |
| LookaheadCm | 350 | 350 (유지) | |
| ArrivalGoalCm | 150 | 150 (유지) | |
| SteerCap | 0.5 | 0.5 (유지) | |
...
**부작용 및 진행 중인 후속 튜닝:** MuLat을 올린 만큼 회전 자체가 둔해져서, 목표
지점 근처에서 방향을 제때 못 꺾고 스쳐 지나가는(overshoot) 현상 발생. 다음 후보로
`ApproachGain`(도착 직전 접근 단계 전용 게인으로 추정, 1.0 → 1.5~2.0 검토)과
`SteerCap`(0.5 → 0.65 검토, 마지막 급조향이 캡에 잘리는지 확인) 조정을 검토 중 —
전체 경로 추종용 `YawGain`은 이미 안정화 목적으로 낮춰둔 값이라 재조정 시 다시
불안정해질 위험이 있어 우선순위 낮음.

---

## 7. 알려진 이슈 / 다음 단계

- [ ] **`VehicleSetup.bUseTopSpeedOverride=true, TopSpeed=11.111 m/s(40km/h)` 적용**
      (교수님 URDF 리뷰로 발견 — Sweep Table 측정 조건과 실제 최고속 제한이 안 맞았음.
      에디터 연결 문제로 세션 중 미적용, 다음 접속 시 우선 적용)
- [ ] `ApproachGain` / `SteerCap` 조정으로 도착 지점 overshoot 개선 (진행 중)
...
- [ ] "에디터 장시간 유지 시 차량 물리 꺼짐" 현상 근본 원인 미확인 (재발 시 에디터
      재시작으로 회피 중)
- [ ] 자율주행 로직을 재사용 가능한 콘솔 커맨드(`MoveUGVGenesisTo` 등)로 정리하는 작업은
      아직 미착수 — 지금은 `BP_UGV_Vehicle_Genesis`의 `EventBeginPlay`에 하드코딩된
      목적지(`ugv_move_point`, 6460,830,0)로만 테스트 중
